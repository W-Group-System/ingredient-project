            ...
            <div class="card border border-1 border-primary">
                <div class="card-header bg-primary card-header-radius">
                    <div class="d-flex align-items-center">
                        <p class="card-title text-white m-0">
                            Advance PR
                            <button class="btn btn-warning" data-toggle="modal" data-target="#advance_pr">Advance PR</button>
                            @include('raw_materials.incoming.pr.advance_pr')
                        </p>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover tablewithSearch">
                            ...
                            <tbody>
                                 @foreach ($advanced_prs as $incoming)
                                     <tr>
                                        <td>
                                            <form action="{{ url('delete_pr', $incoming->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                @csrf
                                                @method('DELETE')
...
<script>
    $(document).ready(function(){
        $('[name="item_code"]').on('change', function(){
            ...
            $.ajax({
                type:"GET",
                url: "{{ url('get_items') }}/"+id,
                success: function(data) {
                    $("[name='item_description']").val(data.ItemName);
                }
            })
        })
    })
